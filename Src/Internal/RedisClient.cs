    ...
    internal class RedisClient : IDisposable
    {
        ...
        private void Connect()
        {
            ...
            try
            {
                ...
                // 如果包含密码，则发送验证
                if (!string.IsNullOrEmpty(_redisConnection.Password)) { SendExpectSuccess(Commands.Auth, _redisConnection.Password.ToUtf8Bytes()); }
                // 指定库时进行选择
                if (_redisConnection.Db != 0) { SendExpectSuccess(Commands.Select, _redisConnection.Db.ToUtf8Bytes()); }
