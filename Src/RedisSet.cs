...
namespace FS.Redis
{
    public class RedisSet
    {
        ...
        /// <param name="vals">member 元素</param>
        ...
        public int Add(string keyName, params string[] vals)
        {
            ...
            Check.IsTure(vals == null || vals.Length == 0, "参数：vals不能为空");
        ...
        {
            Check.IsTure(keyNames == null || keyNames.Length < 2, "参数：keyNames必须大于1个值");

            var lstCmdBytes = new List<byte[]> { Commands.SDiff };
            lstCmdBytes.AddRange(keyNames.Select(o => o.ToUtf8Bytes()));
        ...
        /// </summary>
        /// <param name="keyName">要保存结果的Key名称</param>
        /// <param name="keyNames">Key名称</param>
        /// <returns>集合中元素的数量。</returns>
        public int ExceptSave(string keyName, params string[] keyNames)
        {
            Check.IsTure(keyNames == null || keyNames.Length < 2, "参数：keyNames必须大于1个值");

            var lstCmdBytes = new List<byte[]> { Commands.SDiffStore, keyName.ToUtf8Bytes() };
            lstCmdBytes.AddRange(keyNames.Select(o => o.ToUtf8Bytes()));
            return _redisClient.SendExpectInt(lstCmdBytes.ToArray());
        }
        ...
        /// <param name="keyNames">Key名称</param>
        /// <returns>集合中元素的数量。</returns>
        public int IntersectSave(string keyName, params string[] keyNames)
        {
            Check.IsTure(keyNames == null || keyNames.Length < 2, "参数：keyNames必须大于1个值");

            var lstCmdBytes = new List<byte[]> { Commands.SInterStore, keyName.ToUtf8Bytes() };
            lstCmdBytes.AddRange(keyNames.Select(o => o.ToUtf8Bytes()));
            return _redisClient.SendExpectInt(lstCmdBytes.ToArray());
        }
        ...
        /// <param name="keyNames">Key名称</param>
        /// <returns>集合中元素的数量。</returns>
        public int UnionSave(string keyName, params string[] keyNames)
        {
            Check.IsTure(keyNames == null || keyNames.Length < 2, "参数：keyNames必须大于1个值");

            var lstCmdBytes = new List<byte[]> { Commands.SUnionStore, keyName.ToUtf8Bytes() };
            lstCmdBytes.AddRange(keyNames.Select(o => o.ToUtf8Bytes()));
            return _redisClient.SendExpectInt(lstCmdBytes.ToArray());
        }
        ...
        /// <param name="val">要移动的member</param>
        /// <param name="destinationKeyName">要保存到目标的Key名称</param>
        /// <returns>如果 member 元素被成功移除，返回 1 。。</returns>
        public bool Move(string sourceKeyName, string val, string destinationKeyName)
        {
            Check.IsTure(string.IsNullOrWhiteSpace(sourceKeyName), "参数：sourceKeyName不能为空");
            Check.IsTure(string.IsNullOrWhiteSpace(destinationKeyName), "参数：destinationKeyName不能为空");
            return _redisClient.SendExpectInt(Commands.SMove, sourceKeyName.ToUtf8Bytes(), destinationKeyName.ToUtf8Bytes(), val.ToUtf8Bytes()) > 0;
        }
        ...
        /// </summary>
        /// <param name="keyName">Key名称</param>
        /// <param name="vals">要移除的member 元素</param>
        /// <returns>被成功移除的元素的数量，不包括被忽略的元素。</returns>
        public int Remove(string keyName, params string[] vals)
        {
            Check.IsTure(string.IsNullOrWhiteSpace(keyName), "参数：keyName不能为空");
            Check.IsTure(vals == null || vals.Length == 0, "参数：vals不能为空");
